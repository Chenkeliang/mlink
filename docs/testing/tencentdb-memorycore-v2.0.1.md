...
| L0 capture to cross-session L1 recall | Pass | Ten messages captured in session A became a visible user-scoped L1 memory when recalled from session B in about 15 seconds. Recall does not send `session_id`. |
...
| Two-user L1 isolation | Pass | After user A's L1 canary was confirmed visible in about 21 seconds, user B could not recall it under the same Provider process. |
| Shared-layer policy and hierarchy | Pass | Default recall returned no Agent-scoped items; explicit opt-in saw L2/L3 after about 133 seconds. A second user under the same Agent could read the shared layer; a different Agent could not. |
| Missing identity | Pass | MLink rejected the request locally without falling back to `default`. |
| Invalid live configuration | Pass | Missing URL/token skips the seven backend scenarios; malformed configuration with present credentials fails instead of being silently skipped. |
| Transient inventory | Pass | Ten raw messages containing current stock `104` produced zero L1 memories; L1 extraction completed in about 19 seconds. |

Full final live suite result: 8 tests passed in 223.106 seconds. Every stateful test used a unique service/team/Agent/user fixture. Running without URL/token safely skips all seven backend scenarios while the configuration-validation test still runs and passes.
...
Most recent result on 2026-08-28: **Pass**, 1 test in 17.91 seconds (package elapsed 18.419 seconds).
