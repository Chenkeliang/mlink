...
func TestSnapshotRestorePlanRejectsInsufficientCapacityBeforeCreatingResources(t *testing.T) {
	runner := &snapshotRunner{objects: map[string]bool{}}
	driver := SnapshotDriver{Runner: runner, Target: install.LocalTarget{Runner: runner}, InstanceID: "default", Capacity: func(string) (int64, error) { return 100, nil }}
	...
	if len(runner.commands) != 0 {
		t.Fatalf("capacity preview mutated or queried Docker runtime: %#v", runner.commands)
	}
}
