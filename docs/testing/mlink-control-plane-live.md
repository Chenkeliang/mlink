...
## Blocked by official Panel Dockerfile
...
Because the Panel image cannot be built unchanged, Panel `/health`, public instance listing, and Owner login could not be accepted live. Real-machine Stage A must not be applied until an official image or official Dockerfile fix is selected.
